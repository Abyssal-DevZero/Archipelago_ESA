...
TOKEN_RULES: dict[str, Rule] = {
    ...
    "password": Has(logic.FLAG_EVENTS["password"]),
    "classa": Has(logic.FLAG_EVENTS["classa"]),
}

TELEPORT_ACCESS = Has("Teleport Access")

# The A.I. Mainframe fight. Both endings are self-reported by a client command,
# so there is no event to collect and nothing beyond the graph is required:
# reaching the room is the whole requirement.
...
def set_completion_condition(world: ESAWorld) -> None:
    ...
    if world.options.goal == Goal.option_postgame:
        # Mwyah is not in the graph at all; the postgame asks for the Pillars.
